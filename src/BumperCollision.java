...
public class BumperCollision
{
	...
	public static void collide(Bumper bumper, Ball ball)
	{
		...
		{	
			...
			{
				ball.setX((int) (ball.getX() - (ball.getdx()/10.0 + remainderX)));
				while(remainderX >= 1.0)
					remainderX -= 1.0;
				remainderX += (ball.getdx() % 10.0) / 10.0;
				ball.setY((int) (ball.getY() - (ball.getdy()/10.0 + remainderY)));
				while(remainderY >= 1.0)
					remainderY -= 1.0;
				remainderY += (ball.getdy() % 10.0) / 10.0;
			}
